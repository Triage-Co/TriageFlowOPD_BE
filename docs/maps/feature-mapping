# OPD2.svg Feature Mapping Specification

Bảng ánh xạ các đối tượng đồ họa trong `OPD2.svg` tới các thực thể trong Cơ sở dữ liệu (Prisma Schema).
...
| `Khu_khám_Nội-Nhi` | Khu khám Nội-Nhi | `PED_INT` | Khu khám Nội - Nhi | `polygon#wall-surround-3` <br> + **`line#wall-separate`** |
| `Garden` | Garden | `GARDEN` | Khuôn viên Sân vườn | `rect#wall-garden` |

---

## 3. Danh sách Cửa lẻ / Cửa Khu vực không thuộc Phòng (`Area Non-Room Doors`)

Danh sách 12 cửa khu vực/liên khu (không thuộc bất kỳ phòng khám riêng lẻ nào), được lưu vết thành `Door` và cắt tách ranh giới tường `Boundary` của `Area` tương ứng:
...
| 6 | `door2.4` | (rỗng) | `PED_INT` (Nhà thuốc Nội-Nhi) | `x=2584.31, y=2255.51, w=20, h=180` (rộng 1.8m) | Cửa quầy / Lối vào Nhà thuốc trong Khu Nội-Nhi |
| 7 | `door2-5` | door2 | `PED_INT` (Khu Nội-Nhi) | `x=3294.31, y=3741.57, w=180, h=20` (rộng 1.8m) | Cửa lối vào phía Nam 1 Khu Nội-Nhi |
| 8 | `door2.3` | (rỗng) | `PED_INT` (Khu Nội-Nhi) | `x=4871.31, y=3741.57, w=180, h=20` (rộng 1.8m) | Cửa lối vào phía Nam 2 Khu Nội-Nhi |
| 9 | `door2.2` | (rỗng) | `PED_INT` (Khu Nội-Nhi) | `x=3294.31, y=1829.57, w=180, h=20` (rộng 1.8m) | Cửa vách ngăn phía Bắc 1 Khu Nội-Nhi |
| 10 | `door2.1` | (rỗng) | `PED_INT` (Khu Nội-Nhi) | `x=4871.31, y=1829.57, w=180, h=20` (rộng 1.8m) | Cửa vách ngăn phía Bắc 2 Khu Nội-Nhi |
| 11 | `door1.2` | (rỗng) | `PED_INT` (Khu Nội-Nhi) | `x=7308.31, y=244.07, w=20, h=100` (rộng 1.0m) | Cửa phụ góc Đông-Bắc Khu Nội-Nhi |
| 12 | `door1.1` | (rỗng) | `PED_INT` (Khu Nội-Nhi) | `x=1029.31, y=1608.07, w=20, h=100` (rộng 1.0m) | Cửa phụ góc Tây-Nam Khu Nội-Nhi |

---

## 4. Ánh xạ Phòng chức năng & Lâm sàng (`PhysicalRoom`)

### 4.1 Phòng khám theo mã phòng (Dựa trên `room-list.md`)

| Nhóm SVG (`id`) | Mã phòng CSDL (`roomCode`) | Tên phòng (`roomLabel`) | `RoomType` | Khu vực (`Area`) |
| :--- | :--- | :--- | :--- | :--- |
| `_2.5` - `_2.41` | `G2.2.xx` (vd: `G2.2.35`) | vd: Nhi 2, Sức khỏe tâm thần, Nội tim mạch | `CONSULTATION` | `PED_INT` |
| `_4.1` - `_4.6` | `G2.4.1` - `G2.4.6` | CTCH 1, CTCH 2, Da liễu 1-3 | `CONSULTATION` | `ORTH` / `DERM` |
| `_4.7` - `_4.11` | `G2.4.7` - `G2.4.11` | Mắt 1, Mắt 2,... | `CONSULTATION` | `OPH` |
| `_4.12` - `_4.29` | `G2.4.12` - `G2.4.29` | Ngoại tổng quát, Ngoại thần kinh, RHM | `CONSULTATION` | `SUR` |
| `_4.30` - `_4.32` | `G2.4.30` - `G2.4.32` | Phòng khám Da liễu / Da | `CONSULTATION` | `DERM` |
| `_4.34`, `_4.35` | `G2.4.34`, `G2.4.35` | Đơn vị tiêm chủng / Khám dịch vụ | `CONSULTATION` | Độc lập |

### 4.2 Tiện ích và Phòng đặc thù (Utility Rooms)

| Element SVG (`id`) | `roomCode` | `roomLabel` | `RoomType` | Ghi chú |
| :--- | :--- | :--- | :--- | :--- |
| `rect#Pharmacy-2` | `G2.1.PHARMACY` | Nhà Thuốc / Quầy Thuốc | `PHARMACY` | Nằm trong Khu Nội-Nhi |
| `rect#Receptionist` | `G2.1.RECEPTIONIST` | Quầy Tiếp Nhận | `WAITING` / `OTHER` | Tiếp nhận bệnh nhân Tầng 1 |
| `rect#Reception` | `G2.1.RECEPTION` | Sảnh Tiếp Đón | `WAITING` | Sảnh chờ chính |
| `rect#WC` | `G2.1.WC` | Nhà Vệ Sinh | `OTHER` | Khu WC Tầng 1 |
| `rect#Stairs` | `G2.1.STAIRS` | Cầu Thang Bộ | `OTHER` | Kết nối Tầng 1 - Tầng 2 |
| `rect#Elevators` | `G2.1.ELEVATORS` | Khu Thang Máy | `OTHER` | Thang máy Tòa G2 |
| `rect#empty_room` | `G2.1.EMPTY_1` | Phòng trống 1 | `OTHER` | Thuộc Khu khám mắt |
| `rect#empty_room-2` | `G2.1.EMPTY_2` | Phòng trống 2 | `OTHER` | Thuộc Khu khám da liễu |
