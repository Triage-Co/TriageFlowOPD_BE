...
export class ClinicalDocumentController {
  constructor(private readonly clinicalDocumentService: ClinicalDocumentService) {}
  ...
  @ApiOperation({
    ...
    description: 'Endpoint dành cho Nhân viên y tế (Bác sĩ, Y tá, Kỹ thuật viên, Dược sĩ, Admin) để đính kèm tài liệu lâm sàng mới (Đơn thuốc, kết quả xét nghiệm, chẩn đoán hình ảnh...) vào một phiên khám.',
  })
  @ApiResponse({ status: 201, description: 'Tài liệu lâm sàng đã được tạo thành công.' })
  @ApiResponse({ status: 400, description: 'Dữ liệu đầu vào không hợp lệ.' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy phiên khám bệnh tương ứng với visit_session_id.' })
  create(@Body() createDto: CreateClinicalDocumentReqDto) {
    return this.clinicalDocumentService.create(createDto);
  }

  @Get()
  @roles('DOCTOR', 'NURSE', 'RECEPTIONIST', 'LAB_TECHNICIAN', 'PHARMACIST', 'ADMIN')
  @UseGuards(IsRoleGuard)
  @ApiOperation({
    summary: '[STAFF - ADMIN] Lấy danh sách tất cả tài liệu lâm sàng',
    description: 'Endpoint dành cho Nhân viên y tế tra cứu toàn bộ danh sách các tài liệu lâm sàng trong hệ thống, có hỗ trợ lọc theo ID phiên khám.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Endpoint dành cho Bệnh nhân (USER) để tự xem toàn bộ các tài liệu lâm sàng liên quan đến các phiên khám của chính mình.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Endpoint lấy chi tiết một tài liệu lâm sàng cụ thể. Nhân viên y tế có quyền xem mọi tài liệu, trong khi Bệnh nhân (USER) chỉ có thể xem tài liệu thuộc phiên khám của chính họ.',
  })
  @ApiResponse({ status: 200, description: 'Lấy chi tiết tài liệu lâm sàng thành công.' })
  @ApiResponse({ status: 403, description: 'Bệnh nhân không có quyền xem tài liệu lâm sàng của người khác.' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy tài liệu lâm sàng với ID được cung cấp.' })
  ...
  @ApiOperation({
    ...
    description: 'Endpoint dành cho Nhân viên y tế sửa đổi nội dung hoặc chi tiết của tài liệu lâm sàng (ví dụ sửa đơn thuốc, cập nhật chỉ số xét nghiệm...).',
  })
  @ApiResponse({ status: 200, description: 'Cập nhật tài liệu lâm sàng thành công.' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy tài liệu lâm sàng cần cập nhật.' })
  update(
    @Param('id') id: string,
  ...
  @ApiOperation({
    ...
    description: 'Endpoint giới hạn quyền xóa tài liệu lâm sàng chỉ dành riêng cho Bác sĩ (DOCTOR) và Quản trị viên (ADMIN).',
  })
  @ApiResponse({ status: 200, description: 'Xóa tài liệu lâm sàng thành công.' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy tài liệu lâm sàng cần xóa.' })
