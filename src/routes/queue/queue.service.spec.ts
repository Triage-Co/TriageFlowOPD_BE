...
describe('QueueService isAppointmentOnTime', () => {
  ...
  it('should return true when checkTime is within slot time (including 30 mins early)', () => {
    ...
    expect(isAppointmentOnTime('09:00', '10:00', shiftDate, earlyCheck, timeZone)).toBe(true);

    const exactStart = new Date('2026-08-04T09:00:00+07:00');
    expect(isAppointmentOnTime('09:00', '10:00', shiftDate, exactStart, timeZone)).toBe(true);

    const middleSlot = new Date('2026-08-04T09:30:00+07:00');
    expect(isAppointmentOnTime('09:00', '10:00', shiftDate, middleSlot, timeZone)).toBe(true);
  });
  ...
  it('should return false when checkTime is too early (more than 30 mins before slot start)', () => {
    ...
    expect(isAppointmentOnTime('09:00', '10:00', shiftDate, tooEarly, timeZone)).toBe(false);
  });
  ...
  it('should return false when checkTime is after slot end', () => {
    ...
    expect(isAppointmentOnTime('09:00', '10:00', shiftDate, tooLate, timeZone)).toBe(false);
  });
