...
export class NavigationService {
      ...
      floors.map(async (floor) => {
        ...
        const roomMaps = await Promise.all(
          rooms.map(async (room) => {
            ...
            const boundaries = await this.prisma.roomBoundary.findMany({
            ...
            const boundaryMaps = await Promise.all(
              boundaries.map(async (boundary) => {
                const lineGeom = await this.geoService.readGeom('room_boundary', boundary.id, 'lineGeom');
                return {
                  ...boundary,
                  lineGeom,
                };
  ...
  private async resolveNode(type: RouteLocationType, id: string): Promise<any> {
    ...
    if (type === RouteLocationType.ROOM) {
      ...
      // Find node on this floor where metadata.roomId = room.id
