...
export class PrescriptionController {
  ...
  @ApiOperation({
    ...
    description:
      'Endpoint dành cho Bác sĩ / Dược sĩ khởi tạo đơn thuốc gắn liền với 1 phiên khám (Visit_Session). Tự động khởi tạo Service_Order tương ứng và sinh mã QR đơn thuốc.',
  })
  @ApiResponse({
    status: 201,
    ...
    schema: {
      example: {
        ...
        qr_code:
          '{"code":"RX-20260730-8842","visit_session_id":"...","service_order_id":"...","total_amount":50000}',
        ...
        diagnosis_note:
          'Uống thuốc đúng giờ, nghỉ ngơi nhiều và tái khám sau 7 ngày',
        total_amount: 50000,
        status: 'PENDING',
  ...
  @ApiResponse({
    status: 400,
    description: 'Dữ liệu không hợp lệ hoặc thiếu thông tin thuốc.',
  })
  @ApiResponse({
    status: 404,
    description: 'Không tìm thấy phiên khám hoặc loại thuốc.',
  })
  @ApiResponse({
    status: 409,
    description: 'Phiên khám này đã có đơn thuốc trước đó.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Endpoint hỗ trợ tra cứu danh sách đơn thuốc có thể lọc theo patient_id, visit_session_id, status.',
  })
  @ApiQuery({
    name: 'patient_id',
    required: false,
  ...
  @ApiOperation({
    ...
    description:
      'Endpoint cho phép Dược sĩ / Thu ngân quét mã QR hoặc nhập mã đơn thuốc (VD: RX-20260730-8842) để xem thông tin chi tiết đơn thuốc và số tiền.',
  })
  @ApiParam({
    name: 'code',
    description: 'Mã đơn thuốc (RX-YYYYMMDD-XXXX) hoặc chuỗi QR code',
    example: 'RX-20260730-8842',
  })
  @ApiResponse({
    status: 200,
    description: 'Tra cứu đơn thuốc theo mã thành công.',
  })
  @ApiResponse({
    status: 404,
    description: 'Không tìm thấy đơn thuốc phù hợp.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Endpoint giúp lấy thông tin đơn thuốc duy nhất của một phiên khám.',
  })
  ...
  findByVisitSession(
    ...
    return this.prescriptionService.findByVisitSession(
      visit_session_id,
      req.user,
    );
  }
  ...
  @ApiOperation({
    ...
    description:
      'Endpoint lấy chi tiết 1 đơn thuốc. Nhân viên y tế có thể xem mọi đơn thuốc, Bệnh nhân chỉ xem được đơn thuốc của mình.',
  })
  @ApiResponse({
    status: 200,
    description: 'Lấy chi tiết đơn thuốc thành công.',
  })
  @ApiResponse({
    status: 403,
    description: 'Không có quyền xem đơn thuốc của người khác.',
  })
  ...
  @ApiOperation({
    summary:
      '[STAFF - ADMIN] Xác nhận thanh toán tiền mặt (Offline) tại quầy nhà thuốc',
    description:
      'Chuyển đơn thuốc sang trạng thái PROCESSING (đang soạn thuốc), cập nhật Service_Order.payment_status = SUCCESSED và tự động tạo bản ghi Transaction doanh thu.',
  })
  @ApiResponse({
    status: 200,
    description:
      'Xác nhận thanh toán offline thành công, đơn thuốc chuyển sang PROCESSING.',
  })
  @ApiResponse({
    status: 400,
    description:
      'Đơn thuốc đã được thanh toán hoặc không ở trạng thái PENDING.',
  })
  @ApiResponse({ status: 404, description: 'Không tìm thấy đơn thuốc.' })
  ...
  @ApiOperation({
    ...
    description:
      'Dược sĩ bấm xác nhận đã soạn xong thuốc. Chuyển đơn thuốc sang trạng thái PREPARED và tự động gửi thông báo (Notification) tới ứng dụng của Bệnh nhân.',
  })
  @ApiResponse({
    status: 200,
    description:
      'Xác nhận soạn thuốc xong thành công, đã phát thông báo cho bệnh nhân.',
  })
  @ApiResponse({
    status: 400,
  ...
  @ApiOperation({
    ...
    description:
      'Dược sĩ bấm xác nhận đã giao thuốc. Chuyển đơn thuốc sang trạng thái DISPENSED, chuyển Service_Order sang COMPLETED và tự động gửi thông báo hoàn thành.',
  })
  @ApiResponse({
    status: 200,
    description: 'Xác nhận giao thuốc thành công, hoàn thành đơn thuốc.',
  })
  @ApiResponse({
    status: 400,
    description: 'Đơn thuốc chưa ở trạng thái PREPARED.',
  })
  @ApiResponse({ status: 404, description: 'Không tìm thấy đơn thuốc.' })
  ...
  @ApiResponse({
    status: 200,
    description: 'Cập nhật trạng thái đơn thuốc thành công.',
  })
  @ApiResponse({ status: 404, description: 'Không tìm thấy đơn thuốc.' })
  updateStatus(
    @Param('id') id: string,
    @Body() statusDto: UpdatePrescriptionStatusDto,
  ) {
  ...
  @ApiOperation({
    summary:
      '[DOCTOR - ADMIN] Chỉnh sửa nội dung đơn thuốc (Khi đơn ở trạng thái PENDING)',
    description:
      'Bác sĩ cập nhật dặn dò hoặc thay đổi danh sách thuốc kê khi đơn thuốc chưa được thanh toán/xử lý.',
  })
  @ApiResponse({
    status: 200,
    description: 'Cập nhật nội dung đơn thuốc thành công.',
  })
  @ApiResponse({
    status: 400,
    description: 'Đơn thuốc đã được xử lý, không thể chỉnh sửa.',
  })
  @ApiResponse({ status: 404, description: 'Không tìm thấy đơn thuốc.' })
  ...
  @ApiResponse({
    status: 404,
    description: 'Không tìm thấy đơn thuốc cần xóa.',
  })
