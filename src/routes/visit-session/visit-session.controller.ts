...
export class VisitSessionController {
  ...
  @ApiOperation({
    ...
    description: 'Endpoint dành cho Nhân viên y tế khởi tạo một phiên khám bệnh mới cho bệnh nhân, ghi nhận lý do khám và các chỉ số sinh tồn ban đầu.',
  })
  @ApiResponse({ status: 201, description: 'Phiên khám bệnh đã được tạo thành công.' })
  @ApiResponse({ status: 400, description: 'Dữ liệu yêu cầu không hợp lệ hoặc thiếu trường bắt buộc.' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy bệnh nhân tương ứng với patient_id cung cấp.' })
  ...
  @ApiOperation({
    ...
    description: 'Endpoint dành cho Nhân viên y tế tra cứu danh sách toàn bộ các phiên khám bệnh trong hệ thống, hỗ trợ lọc theo ID bệnh nhân.',
  })
  @ApiQuery({ name: 'patient_id', required: false, description: 'Lọc theo ID bệnh nhân' })
  @ApiResponse({ status: 200, description: 'Lấy danh sách các phiên khám bệnh thành công.' })
  ...
  @ApiOperation({
    ...
    description: 'Endpoint dành cho Bệnh nhân (USER) để tự tra cứu toàn bộ lịch sử các phiên khám bệnh của bản thân dựa trên token đăng nhập.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Endpoint lấy thông tin chi tiết một phiên khám bệnh. Nhân viên y tế có thể xem mọi phiên khám, nhưng Bệnh nhân (USER) chỉ có thể xem nếu phiên khám đó thuộc về chính họ.',
  })
  @ApiResponse({ status: 200, description: 'Lấy chi tiết phiên khám bệnh thành công.' })
  @ApiResponse({ status: 403, description: 'Bệnh nhân không có quyền truy cập vào phiên khám của người khác.' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy phiên khám với ID được cung cấp.' })
  ...
  @ApiOperation({
    ...
    description: 'Endpoint dành cho Nhân viên y tế cập nhật thông tin phiên khám bệnh (chẩn đoán lâm sàng, chỉ số sinh tồn, bệnh sử, khám thực thể...).',
  })
  @ApiResponse({ status: 200, description: 'Cập nhật phiên khám bệnh thành công.' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy phiên khám bệnh cần cập nhật.' })
  update(
    @Param('id') id: string,
    @Body() updateDto: UpdateVisitSessionReqDto,
  ) {
  ...
  @ApiOperation({
    ...
    description: 'Endpoint giới hạn chỉ dành cho Quản trị viên (ADMIN) để thực hiện xóa bỏ phiên khám bệnh ra khỏi hệ thống.',
  })
  @ApiResponse({ status: 200, description: 'Xóa phiên khám bệnh thành công.' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy phiên khám bệnh cần xóa.' })
